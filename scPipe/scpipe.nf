...
// nextflow run scpipe.nf -with-report scPipe.html -with-dag scPipe.png
// Params
...
process Trim_Barcode{
  ...
  output:
  set name, file("${name}.trimmed.fastq") into fqTrimmed
  file ("${name}.*")

  """
  Rscript ${scriptR} Trim_Barcode ${params.PlatForm} ${read2} ${read1} \$PWD/${name}.trimmed.fastq
  """
}
